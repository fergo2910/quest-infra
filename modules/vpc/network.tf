...
resource "aws_route_table" "database" {
  count    = length(var.db_subnet_cidrs) > 0 ? local.nat_gw_count : 0
  provider = aws
  vpc_id   = aws_vpc.this.id
  tags = merge(
    {
      "Name" = "${var.environment}-rt-database_subnets-${count.index + 1}"
    },
    var.tags,
  )
}
...
###
# Associate database route table(s) with database subnets
###
